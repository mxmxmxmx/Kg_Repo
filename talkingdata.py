...
############################
cols =['ip', 'os', 'app', 'channel', 'day', 'hour', 'minute', 'second']
summary = {}
for col in cols:
    summary[col]=train_df[[col]+['is_attributed']].groupby(by=[ 'is_attributed']).describe()
...
gp = train_df[['ip','day','hour','channel']].groupby(by=['ip','day','hour'])[['channel']].count().reset_index().rename(index=str, columns={'channel': 'ip_day_hour_count_channel'})
...
print('grouping by ip-device combination...')
gp = train_df[['ip', 'device', 'channel']].groupby(by=['ip', 'device'])[['channel']].count().reset_index().rename(index=str, columns={'channel': 'ip_device_count_channel'})
...
print('grouping by : ip_device_os_var_hour')
gp = train_df[['ip','device', 'os', 'hour']].groupby(by=['ip', 'device', 'os'])[['hour']].var().reset_index().rename(index=str, columns={'hour': 'ip_device_os_var_hour'})
...
train_df.info()
train_df['ip_day_hour_count_channel'] = train_df['ip_day_hour_count_channel'].astype('uint16')
